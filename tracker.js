...
exports.newTracker = () => {
  ...
  exports2.computeDistance = iouDistance;

  exports2.updateTrackedItemsWithNewFrame = function (detectionsOfThisFrame, frameNb) {
  ...
  exports2.setParams = function (newParams) {
    Object.keys(newParams).forEach((key) => {
      params[key] = newParams[key];
    });
  }

  exports2.enableKeepInMemory = function () {
    keepAllHistoryInMemory = true;
  }

  exports2.disableKeepInMemory = function () {
    keepAllHistoryInMemory = false;
  }

  exports2.getJSONOfTrackedItems = function (roundInt = true) {
    return Array.from(mapOfItemsTracked.values()).map(function (itemTracked) {
      return itemTracked.toJSON(roundInt);
    });
  };

  exports2.getJSONDebugOfTrackedItems = function (roundInt = true) {
    return Array.from(mapOfItemsTracked.values()).map(function (itemTracked) {
      return itemTracked.toJSONDebug(roundInt);
    });
  };

  exports2.getTrackedItemsInMOTFormat = function (frameNb) {
    return Array.from(mapOfItemsTracked.values()).map(function (itemTracked) {
      return itemTracked.toMOT(frameNb);
    });
  };

  exports2.getTrackedItems = function() {
    return mapOfItemsTracked;
  }

  // Work only if keepInMemory is enabled
  exports2.getAllTrackedItems = function () {
    return mapOfAllItemsTracked;
  };

  // Work only if keepInMemory is enabled
  exports2.getJSONOfAllTrackedItems = function () {
